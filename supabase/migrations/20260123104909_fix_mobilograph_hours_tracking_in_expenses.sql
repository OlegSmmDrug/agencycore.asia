/*
  # Исправление учета часов мобилографа в расходах проекта

  ## Описание
  Исправляет функцию process_completed_task() для правильного учета часов
  мобилографа при первом создании записи в project_expenses.

  ## Проблема
  При создании новой записи в project_expenses функция устанавливала только
  production_expenses, но не заполняла production_mobilograph_hours.

  ## Изменения
  - При создании записи project_expenses добавляется логика по должности
  - Часы мобилографа, фотографа и видеографа корректно записываются
*/
  ...
  IF NEW.status = 'Done' AND (OLD.status IS NULL OR OLD.status != 'Done') THEN
    ...
    IF NEW.project_id IS NOT NULL THEN
      ...
      IF v_project_expenses.id IS NULL THEN
        -- Создаем новую запись расходов в зависимости от должности
        CASE v_user_job_title
          ...
          ELSE
            -- Для остальных должностей только production_expenses
            INSERT INTO project_expenses (
              project_id,
              month,
              organization_id,
