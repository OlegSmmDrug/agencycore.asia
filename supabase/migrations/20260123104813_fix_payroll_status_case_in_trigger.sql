/*
  # Исправление регистра статуса в триггере payroll

  ## Описание
  Исправляет триггер process_completed_task() - меняет 'draft' на 'DRAFT'
  для соответствия constraint в таблице payroll_records.

  ## Проблема
  Constraint требует status в верхнем регистре ('DRAFT', 'FROZEN', 'PAID'),
  а функция использовала нижний регистр ('draft').

  ## Изменения
  - Обновлена функция process_completed_task()
  - Изменен статус с 'draft' на 'DRAFT'
*/
...
BEGIN
  ...
  IF NEW.status = 'Done' AND (OLD.status IS NULL OR OLD.status != 'Done') THEN
    ...
    IF v_payroll_record.id IS NULL THEN
      -- Создаем новую запись со статусом DRAFT (верхний регистр!)
