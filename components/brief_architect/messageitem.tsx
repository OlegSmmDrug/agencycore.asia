...
import { BriefMessage } from './types';
...
export const MessageItem: React.FC<MessageItemProps> = ({ message }) => {
  ...
  formattedContent = formattedContent.replace(/\[Прогресс.*?\]\s*\|\s*\[Статус.*?\]/, '').trim();
  formattedContent = formattedContent.replace(/<json_brief>[\s\S]*?<\/json_brief>/, '').trim();
    ...
    <div className={`flex ${isAI ? 'justify-start' : 'justify-end'} mb-8 animate-in slide-in-from-bottom-2 duration-300`}>
      <div 
        className={`max-w-[85%] rounded-xl p-5 shadow-sm transition-all ${
          isAI
            ? 'bg-white border-l-4 border-l-blue-600 border-y border-r border-slate-100 text-slate-800 rounded-tl-none'
      ...
      >
        {isAI && (
          <div className="flex items-center space-x-2 mb-3 border-b border-slate-50 pb-2">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-blue-600">Strategic Insight</span>
          </div>
        )}
