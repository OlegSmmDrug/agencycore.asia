...
const SalarySchemes: React.FC<SalarySchemesProps> = ({ users, schemes, onUpdateScheme, availableJobTitles = [] }) => {
    ...
    const handleResetScheme = () => {
        const emptyScheme: SalaryScheme = {
            id: foundScheme?.id || `sch_${activeTarget}`,
            targetId: activeTarget,
            targetType: selectedTab === 'titles' ? 'jobTitle' : 'user',
            baseSalary: 0,
            kpiRules: [],
        ...
        <div className="flex h-full bg-white animate-fade-in relative">
            ...
            `}>
                <div className="p-4 flex bg-slate-50 border-b border-slate-100">
                    ...
                    <button
                        onClick={() => { setSelectedTab('users'); if (users.length > 0) setActiveTarget(users[0].id); }}
                ...
                {selectedTab === 'users' && users.length === 0 ? (
                    <div className="flex items-center justify-center h-full">
                        <div className="text-center">
                            <p className="text-slate-400 font-bold text-sm">Нет сотрудников</p>
                            <p className="text-xs text-slate-300 mt-1">Добавьте сотрудников во вкладке "Участники"</p>
                        </div>
                    </div>
                ) : (
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-3">
                                <h2 className="text-lg md:text-2xl font-black text-slate-900 tracking-tight uppercase">
                                    Схема: {selectedTab === 'users' ? users.find(u => u.id === activeTarget)?.name || activeTarget : activeTarget}
                                </h2>
                                {saveStatus === 'saving' && (
                                    <span className="flex items-center gap-1 text-[10px] font-bold text-blue-500">
                                        <span className="w-3 h-3 border-2 border-blue-300 border-t-blue-600 rounded-full animate-spin"></span>
                                    </span>
                                )}
                                {saveStatus === 'saved' && (
                                    <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-500 animate-fade-in">
                        ...
                        {foundScheme && (
                            <button
                                ...
                                className="text-[10px] font-black text-slate-400 hover:text-red-500 border border-slate-200 hover:border-red-200 px-3 py-1.5 rounded-lg transition-all uppercase"
                            >
                                Сбросить
                            </button>
                        )}
            ...
            <ConfirmDialog
                ...
                onConfirm={handleResetScheme}
                title="Сбросить схему?"
                message="Все настройки (фикс, KPI-ставки) для этой схемы будут обнулены. Правила мотивации (бонусы) не будут затронуты."
