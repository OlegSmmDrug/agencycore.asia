...
const ProjectBoard: React.FC<ProjectBoardProps> = ({
  ...
  const [selectedExecutorId, setSelectedExecutorId] = useState<string>('all');
  ...
  const executorUsers = users.filter(u =>
    projects.some(p => p.teamIds && p.teamIds.includes(u.id))
  );
  ...
  const filteredProjects = projects.filter(project => {
    ...
    const matchesExecutor = selectedExecutorId === 'all' || (project.teamIds && project.teamIds.includes(selectedExecutorId));

    if (!searchQuery.trim()) return matchesExecutor;
    ...
    const matchesClientName = client?.name?.toLowerCase().includes(query) || client?.company?.toLowerCase().includes(query);
    const matchesService = project.services?.some(s => s.toLowerCase().includes(query));
    const matchesStatus = config?.label?.toLowerCase().includes(query);

    return (matchesName || matchesClientName || matchesService || matchesStatus) && matchesExecutor;
  });
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      {project.imageUrl && (
                        <img src={project.imageUrl} alt={project.name} className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg object-cover flex-shrink-0" />
                      )}
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 pl-0 sm:pl-0">
                      ...
                      {onRestoreProject && (
                        <button
                          ...
                          className="px-2 sm:px-3 py-1.5 text-xs font-bold text-green-600 bg-green-50 hover:bg-green-100 border border-green-200 rounded-lg transition-colors flex items-center gap-1"
                        >
                      ...
                      {onDeleteProject && (
                        <button
                          ...
                          className="px-2 sm:px-3 py-1.5 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg transition-colors flex items-center gap-1"
                        >
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    return (
      <div
        ...
        className={`p-4 rounded-xl border cursor-pointer hover:shadow-md transition-all ${
          overdueDays ? 'border-red-200 bg-red-50/30' : 'border-slate-200 bg-white'
        }`}
      >
        <div className="flex items-start gap-3 mb-3">
          {project.imageUrl && (
          ...
          <div className="flex-1 min-w-0">
            <h4 className="font-bold text-slate-800 text-sm truncate">{project.name}</h4>
            {client && <p className="text-xs text-slate-500 truncate">{client.company || client.name}</p>}
          </div>
          <button
            onClick={(e) => onEditProject(project, e)}
        ...
        <div className="flex items-center gap-2 flex-wrap mb-3">
          {level1Stage && level1Stage.stage ? (
            <div
              className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-white font-bold text-[10px]"
              style={{ backgroundColor: level1Stage.stage.color }}
            >
              <span>{level1Stage.stage.icon}</span>
              <span>{level1Stage.stage.name}</span>
          ...
          <span className="text-[10px] font-bold text-slate-600">{project.budget.toLocaleString()} ₸</span>
          {agencyRevenue !== project.budget && (
            <span className="text-[10px] text-green-600 font-medium">{agencyRevenue.toLocaleString()} ₸</span>
          )}
          {overdueDays && (
            <span className="text-[10px] font-bold text-red-500">-{overdueDays}д</span>
          )}
        </div>
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
      <div className="overflow-auto custom-scrollbar flex-1">
        <div className="lg:hidden p-4 space-y-3">
          {filteredProjects.map(project => renderListCard(project))}
          {filteredProjects.length === 0 && (
      ...
      <div className="flex flex-col gap-3 mb-4 shrink-0">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <h1 className="text-lg sm:text-xl font-bold text-slate-900 whitespace-nowrap">Проекты</h1>
            <span className="text-xs font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded-lg border border-slate-200 whitespace-nowrap">
              {filteredProjects.length} из {projects.length}
            </span>
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
            {viewScope !== 'archive' && (
              <div className="flex bg-slate-100 p-1 sm:p-1.5 rounded-xl border border-slate-200 gap-0.5 sm:gap-1">
                <button onClick={() => setViewType('board')} className={`p-2 sm:p-2.5 rounded-lg transition-all ${viewType === 'board' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`} title="Канбан по этапам">
                  <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
                </button>
                <button onClick={() => setViewType('services')} className={`p-2 sm:p-2.5 rounded-lg transition-all ${viewType === 'services' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`} title="Канбан по услугам">
                  <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2" /></svg>
                </button>
                <button onClick={() => setViewType('list')} className={`p-2 sm:p-2.5 rounded-lg transition-all ${viewType === 'list' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`} title="Список">
                  <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>
                </button>
              </div>
            )}
            <button onClick={onAddProject} className="bg-blue-600 text-white px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl text-xs font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 active:scale-95 flex items-center gap-2">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex items-center gap-2 overflow-x-auto whitespace-nowrap pb-1 sm:pb-0 -mb-1 sm:mb-0">
            {currentUserId && (
              <button
          ...
          <div className="hidden sm:block w-px h-6 bg-slate-200 flex-shrink-0"></div>
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <div className="relative flex-1 min-w-0 max-w-md">
              ...
              <input
                ...
                placeholder="Поиск по названию, клиенту, услуге..."
                className="w-full pl-9 pr-8 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all placeholder:text-slate-400"
              />
              {searchQuery && (
                <button
            ...
            <select
              ...
              className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-600 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 cursor-pointer flex-shrink-0 max-w-[160px] sm:max-w-[200px]"
            >
              <option value="all">Все исполнители</option>
              {executorUsers.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
            </select>
          </div>
        </div>
      </div>
